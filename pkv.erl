...
read(Key) ->
	log_c("Reading Value..."),
	N = message_all_servers({read, Key, self()}),
...
server_loop([]) -> %Init routine for the server loop. Cleaner than defining defaults in create_server().
	...
	Proposals = [],
	server_loop({Dict, Proposals});
server_loop({Dict, Proposals}) ->
	NewState = receive
		{read, Key, Client} ->
			log("replying to read request for key", [Key]),
			CurrentValue = case dict:find(Key, Dict) of
                {ok, Value} -> Value;
                error       -> not_found
            end,			
			Client ! {read_value, self(), CurrentValue},
			{Dict, Proposals};
		{write, WriteValue, From} ->
			log("initiating write request:", [WriteValue]),
			Result =  handle_write_request(WriteValue),
			From ! Result,				
			{Dict, Proposals};
		{prepare, ProposedSeq, ProposedValue, From} ->
			...
			case ProposedSeq > HighestSeq of
				true ->
					...
					{Dict, [{ProposedSeq, ProposedValue}|Proposals]}; %Add this promise to our list of proposals.
				false ->
...
send_decision(Key, Value) ->
	message_all_servers({decision, Key, Value}).
...
all_test() ->
	...
	K1 = write(testing_testing),
	K2 = write("Cole"),
	K3 = write({balance, 123.45}),
	timer:sleep(50), % Wait for the paxos round to complete...
	?assertEqual({testing_testing, 3}, read(K1)),
	?assertEqual({"Cole", 3}, read(K2)),
	?assertEqual({{balance, 123.45}, 3}, read(K3)),
	stop().
